%wykres błedów względnych dla całki z sin(2*x) - cos(x) na przedziale [0.001,b], gdzie
...
I_analytical = arrayfun(@(b) integral(f3, a, b), b_values);
...
for i = 1:length(b_values)
    I_numerical = accuracy_simpson_integration(f3, a, b_values(i), 1e-5);
    rel_errors_1e5(i) = abs(I_analytical(i) - I_numerical) / abs(I_analytical(i));
    I_numerical = accuracy_simpson_integration(f3, a, b_values(i), 1e-1);
...
figure;
plot(b_values, rel_errors_1e5, 'r-', 'LineWidth', 3.5, 'DisplayName', 'Dokładność 1e-5');
hold on;
...
title('Błąd względny w metodzie Simpsona dla sin(2*x) - cos(x) na [0.001, b] przy różnych dokładnościach', 'FontSize', 22);
legend show;
legend('FontSize', 18);
grid on;
